...
4. Move to that line origin at Z10, lower vertically to the original KAMP
   0.80 mm purge height, run the part-cooling output at full speed, and wait
...
5. Restore the previous 4 mm retract and reproduce the original configured KAMP
   purge: move 18 mm while extruding 18 mm of filament at Z0.80, using KAMP's
   original flow-rate speed calculation. Move 2 mm farther without extrusion,
   lower vertically to Z0.20, then move another 20 mm at 25 mm/s with extrusion
   calculated for a fixed 0.45 mm line width.

The complete 40 mm path is clamped inside the printable limits. The macro also
...
Initial XY positioning occurs at the configured 10 mm travel height. The nozzle
lowers vertically to Z0.80 at the purge origin, and later lowers to Z0.20 only
after moving 2 mm clear of the heavy purge line. The macro rejects zero or
negative purge heights.
